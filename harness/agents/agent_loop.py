...
2. 从 workspace 读取 SOUL.md 作为 system_prompt
...
class HarnessShellBackend(LocalShellBackend):
    ...
    def execute(self, command: str, *, timeout: int | None = None):
        # Replace /workspaces/ virtual paths with real absolute paths
        resolved_command = command.replace("/workspaces/", f"{self.cwd}/workspaces/")
...
def _load_prompt(agent_cfg: AgentConfig, workspace_dir: str, harness_root: str) -> str | None:
    ...
    - agent_cfg.prompt 为空：使用 SOUL.md + harness prompt（默认行为）
    ...
    - 都不设置 → 使用 SOUL.md + harness prompt
    """
    prompt_path_str = agent_cfg.prompt or ""

    if prompt_path_str:
        # 自定义 prompt 文件：替代默认的 SOUL.md + harness prompt
    ...
    # 默认行为：SOUL.md + harness prompt
    soul = _load_soul(workspace_dir)
...
def _load_soul(workspace_dir: str) -> str | None:
    """从 workspace 读取 SOUL.md"""
    soul_path = Path(workspace_dir) / "SOUL.md"
    if soul_path.exists():
...
def build_agent(
    ...
    # 3. Backend（HarnessShellBackend 支持 execute 工具，virtual_mode=True 让 /workspaces/... 映射到 harness_root/workspaces/...）
    ...
    # 8. Middleware: Tool-Filter + Micro-Compact + Manual-Compact
    ...
    agent = create_deep_agent(
        ...
        middleware=[tool_filter, micro_compact, manual_compact],
