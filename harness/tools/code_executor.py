...
Agent 先用 write_file 把代码写到 /workspaces/<agent>/code/ 下，再用本工具执行。
...
实际映射到 harness_root/workspaces/main/code/fib.py。
"""
...
def _resolve_virtual_path(virtual_path: str) -> Path:
    ...
    # 去掉开头的 / ，拼接到 harness_root
    relative = virtual_path.lstrip("/")
    return Path(harness_root) / relative


@tool
def execute_code(file_path: str, args: str = "", timeout: int = 120) -> str:
    ...
    Args:
        ...
        args: Command-line arguments to pass to the script (e.g. "--input /path/a.json --output /path/b.json").
...
def _resolve_args(args: str) -> str:
    ...
    for token in tokens:
        if token.startswith("/workspaces/"):
            ...
            resolved.append(str(Path(harness_root) / relative))
        else:
