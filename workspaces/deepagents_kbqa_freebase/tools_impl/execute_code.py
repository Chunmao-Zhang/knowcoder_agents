...
def execute_code(
    ...
    if not class_code:
        return (
            ...
            "The schema_file may contain only schema-level placeholders without entity MIDs."
        )
    ...
    if total_entity_count > MAX_ENTITY_FOR_FULL and triplets:
        ...
        # Also extract any MID the LLM explicitly references in its code
        for m in re.findall(r"m\.\w+|g\.\w+", code):
            anchor_mids.add(m)
        if anchor_mids:
            ...
            for h, p, t in triplets:
                if str(h) not in reachable:
                    continue
                tail_is_entity = isinstance(t, str) and (t.startswith("m.") or t.startswith("g."))
            ...
            for h, p, t in triplets:
                kept_mids.add(str(h))
                if isinstance(t, str) and (t.startswith("m.") or t.startswith("g.")):
                    kept_mids.add(t)
            type_map = {
...
def _instantiate_all(type_map, mid_name_map, triplets, ontology):
    ...
    cls_is_entity = {{}}
    for cls_name, mids in type_map.items():
        cls_is_entity[cls_name] = bool(mids) and all(
            isinstance(t, str) and (t.startswith("m.") or t.startswith("g."))
            for t in mids
        )
    ...
    def get_unified_instance(mid, default_type):
        if mid not in unified_instances:
            ...
            if not bases:
                class DummyEntity:
                    def __init__(self, _mid=None):
                        self._mid = _mid
                bases.append(DummyEntity)
            
            try:
                CombinedType = type(f"Combined_{{str(mid).replace('.', '_')}}", tuple(bases), {{}})
                obj = CombinedType(_mid=mid)
            except Exception:
                obj = bases[0](_mid=mid)
    ...
    for h_mid, predicate, t_mid in triplets:
        ...
        rel_attr = predicate.replace('.', '_').replace('-', '_')
        ...
        for head_type, tail_type in pairs:
            ...
            tail_is_entity = isinstance(t_mid, str) and (t_mid.startswith("m.") or t_mid.startswith("g."))
...
class EntityList(list):
    def __getitem__(self, key):
        if isinstance(key, str):
            for entity in self:
                if getattr(entity, "_mid", None) == key:
                    return entity
            raise KeyError(key)
...
entities = EntityList([e for e in entities if hasattr(e, "_mid")])
entities_by_mid = {{e._mid: e for e in entities if getattr(e, "_mid", None)}}
...
def get_name(entity):
    if entity is None:
        return ""
    if hasattr(entity, "_mid") and entity._mid:
        return mid_name_map.get(entity._mid, "")
    return ""
...
"""
    ...
    try:
        try:
            ...
            if entity_count > 500:
                hint = (
                    ...
                    "Use `entity = entities_by_mid['m.xxx']` to access topic/parent entities directly "
        ...
        if isinstance(res_obj, dict):
            ...
            # ── CVT 节点检测（对齐 CoG pipeline Step 8）──
            # 两层检测：
            #   1. mid_name_map 层面：_name 为空 / "CVT node" → 一定是 CVT
            #   2. SPARQL 层面：mid_name_map 有名字但 SPARQL 查不到 → 可能是 CVT
            #   CoG 用 fetch_entity_name(first_mid) 做这层验证
            if detailed and isinstance(detailed, dict):
                filtered = {}
                cvt_removed = []
                # 批量查一次 SPARQL 来验证 mid 真实性
                try:
                    from .build_subgraph_schema import _get_entity_names_bulk
                    mids_to_check = [k for k in detailed.keys() if str(k).startswith(("m.", "g."))]
                    sparql_names = _get_entity_names_bulk(mids_to_check) if mids_to_check else {}
                except Exception:
                    sparql_names = {}
                ...
                for mid_key, info in detailed.items():
                    ...
                    if isinstance(info, dict):
                        name = info.get("_name", "")
                    # CVT 判定：(1) _name 空/CVT node  (2) mid_name_map 里没有  (3) SPARQL 也查不到名字
                    local_name = mid_name_map.get(mid_key, "")
                    sparql_name = sparql_names.get(mid_key, "")
                    is_cvt = (
                        (not name or name == "CVT node")
                        and (not local_name or local_name == "CVT node")
                    ) or (
                        str(mid_key).startswith(("m.", "g."))
                        and not sparql_name
                        and (not local_name or local_name == "CVT node")
                    )
                    if is_cvt:
            ...
            # Category 3: Key error (non-MID keys)
            ...
            if detailed and isinstance(detailed, dict) and not code_hint:
                ...
                all_are_mids = all(str(k).startswith(("m.", "g.")) for k in all_mids)
                if not all_are_mids:
                    bad_keys = [k for k in all_mids if not str(k).startswith(("m.", "g."))][:3]
                    code_hint = (
                        f"[CODE_HINT] Key error (category 3): keys in detailed_results should be "
                        f"entity MIDs (like 'm.xxx'), but got: {bad_keys}. "
