...
0-TE: vector-search active-graph predicate candidates by semantic similarity.
"""
...
class SearchPredicateTool(BaseTool):
    """Vector-search active generic graph predicate candidates.

    Returns top_k (predicate / description) pairs.
    Used in 0-TE subtasks to identify predicates first, then use search_entity_by_predicate to look up entity names.
    """

    name: str = "search_predicate"
    description: str = (
        "Vector-search active graph predicate candidates. Returns top_k (predicate / description) pairs. "
        "Used in 0-TE subtasks to identify predicates first, then use search_entity_by_predicate to look up entity names."
    )
